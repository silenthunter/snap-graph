...
void aggc_print_communities(aggc_comm_t *communities, attr_id_t n, weight_t L) {
    attr_id_t i, j;
...
void aggc_print_final_communities(aggc_comm_t *communities, aggc_maxheap_t *maxheap, weight_t L) {
    attr_id_t i, j, k;
...
void aggc_maxheap_check(aggc_maxheap_t *maxheap) {
...
void aggc_maxkey_comm_check(aggc_comm_t *communities, attr_id_t comm1) {
...
void aggc_maxkey_check(aggc_comm_t *communities, aggc_maxheap_t *maxheap) {
    attr_id_t i, j, k;
    double x, max_val; 
...
void aggc_maxheap_sift_up(aggc_maxheap_t *maxheap, attr_id_t idx) {
...
void aggc_maxheap_sift_down(aggc_maxheap_t *maxheap, attr_id_t idx) {
...
void aggc_maxheap_remove(aggc_maxheap_t *maxheap, attr_id_t comm_id) {
...
attr_id_t aggc_adjcomm_pos(aggc_adjcomm_t *adjcomm, attr_id_t val, attr_id_t degree) {
...
void aggc_adjcomm_resize(aggc_comm_t *communities, attr_id_t comm_id, attr_id_t incr) {
...
void aggc_update_values(aggc_comm_t *communities, attr_id_t comm1, attr_id_t idx,
...
void aggc_update_dq_p1(aggc_comm_t *communities, aggc_maxheap_t *maxheap,
...
void aggc_update_dq_p2(aggc_comm_t *communities, aggc_maxheap_t* maxheap,
...
void aggc_remove_adjcomm(aggc_comm_t *communities, aggc_maxheap_t *maxheap,
    ...
    switch (keytype) {
        ...
        case KEY_MB:
            ...
            fprintf(stderr,"DEBUG_STATS stop e/L: %9.6f  mkey: %9.6f stat: %9.6f  mdq: %9.6f\n",
                    (double)edgeCnt/(double)(2*L),
                    max_key,
                    dqnorm_mean + 2.5*dqnorm_stddev,
                    max_dq);
            if ( (max_key < dqnorm_mean + 2.5*dqnorm_stddev) || (max_dq < 0) )
                return max_dq; 
#else 
            if (max_dq < 0) {
#ifdef VERBOSE
            ...
            if (max_key < INFTY) {
                ...
                fprintf(stderr,"stop e/L: %9.6f  mkey: %9.6f stat: %9.6f  mdq: %9.6f\n",
                        (double)edgeCnt/(double)(2*L),
                        max_key,
                        MKH_mean - 2.5*MKH_stddev,
                        max_dq);
#endif

#ifdef DO_STOPPING_LIJ
                if (MaxKeyHistoryFilled && (max_key < MKH_mean - 2.5*MKH_stddev)) {
#ifdef VERBOSE
                    fprintf(stderr,"Stopping: max_key: %f < mean - 2.5 s.d.'s\n", max_key); 
#endif
                    return max_dq;
                }
#endif
...
attr_id_t aggc_compute_membership(aggc_comm_t *communities, attr_id_t *membership, attr_id_t n) {
