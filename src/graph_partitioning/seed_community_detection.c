...
int comp_comm2(const void *v1, const void *v2) {
...
int comp_int2 (const void* v1, const void *v2) {
	const int *p1, *p2;
	p1 = (int*)v1;
	p2 = (int*)v2;
...
void aggc_print_communities2(aggc_comm_t *communities, attr_id_t n, weight_t L) {
    attr_id_t i, j;
...
void aggc_print_final_communities2(aggc_comm_t *communities, aggc_maxheap_t *maxheap, weight_t L) {
    attr_id_t i, j, k;
...
void aggc_maxheap_check2(aggc_maxheap_t *maxheap) {
    ...
    for (i=0; i<n/2; i++) {
        if (2*i+1<n) {
					/* if both root and the child are one of seed vertices or neither is a seed vertex, check whether
						 the parent val is greater than the child */
					if ((heap[i].is_seed == 1 && heap[2*i+1].is_seed == 1)
							|| (heap[i].is_seed == 0 && heap[2*i+1].is_seed == 0)) {
            if ( (fpLT(heap[i].val,heap[2*i+1].val)) 
                ...
                broken=1;
					...
					/* if child is a seed vertex and the parent is not, the seed priority property is violated */
					else if (heap[i].is_seed == 0 && heap[2*i+1].is_seed == 1){
							fprintf (stderr, "ERROR: Seed priority violated (n: %d i: %d 2i+1: %d) heap[i].is_seed equals %d and heap[2i+1].is_seed equals %d\n",
											 n, i, 2*i+1, heap[i].is_seed, heap[2*i+1].is_seed);
							broken = 1;
					}
        }
        if (2*i+2<n) {
					/* if both root and the child are one of seed vertices or neither is a seed vertex, check whether
						 the parent val is greater than the child */
					if ((heap[i].is_seed == 1 && heap[2*i+2].is_seed == 1)
							|| (heap[i].is_seed == 0 && heap[2*i+2].is_seed == 0)) {
            if ( (fpLT(heap[i].val,heap[2*i+2].val)) 
                ...
                broken=1;
					...
					/* if child is a seed vertex and the parent is not, the seed priority property is violated */
					else if (heap[i].is_seed == 0 && heap[2*i+2].is_seed == 1){
							fprintf (stderr, "ERROR: Seed priority violated (n: %d i: %d 2i+2: %d) heap[i].is_seed equals %d and heap[2i+2].is_seed equals %d\n",
											 n, i, 2*i+2, heap[i].is_seed, heap[2*i+2].is_seed);
							broken = 1;
					}
        }
    }
    ...
    if (broken) {
        ...
        for (i=0; i<n; i++) {
            fprintf(stderr, "heap[%4d].is_seed(%d), val (%9.6f) "
#ifdef DO_HEAP2
                    "val2 (%9.6f) "
#endif
                    "comm_id (%4d)\n", 
                    i, heap[i].is_seed, heap[i].val,
#ifdef DO_HEAP2
                    heap[i].val2,
#endif
                    heap[i].comm_id);
...
#ifdef DEBUG
    for (i=0 ; i<n ; i++)
        fprintf(stderr,"DEBUG maxheap val[%5d]: %9.6f .is_seed = %d\n",i,heap[i].val, heap[i].is_seed);
#endif

    return;
}

void aggc_maxkey_comm_check2(aggc_comm_t *communities, attr_id_t comm1) {
...
void aggc_maxkey_check2(aggc_comm_t *communities, aggc_maxheap_t *maxheap) {
    attr_id_t i, j, k;
    double x, max_val; 
...
void aggc_maxheap_sift_up2(aggc_maxheap_t *maxheap, attr_id_t idx) {
    ...
    if (!maxheap->use2) {
        ...
        while (root > 0) {
            parent = (root-1)/2;
						/* if both the child and parent are seed vertices or none of them is a seed
							 swap them based on the value they hold  */
						if ((heap[parent].is_seed == 1 && heap[root].is_seed == 1)
									|| (heap[parent].is_seed == 0 && heap[root].is_seed == 0)) {
            	if (heap[parent].val < heap[root].val) {
                /* swap */
                tmp 			= heap[root].val;
                v   			= heap[root].comm_id;
								is_v_seed = heap[root].is_seed;
                u   			= heap[parent].comm_id;
								is_u_seed = heap[parent].is_seed;

                heap[root].val = heap[parent].val;
                heap[root].comm_id = u;
								heap[root].is_seed = is_u_seed;

                heap[parent].val = tmp;
                heap[parent].comm_id = v;
								heap[parent].is_seed = is_v_seed;
                ...
                root = parent;
					...
					else if (heap[parent].is_seed == 0 && heap[root].is_seed == 1) {
						/*child is one of the seed vertices and parent is not a seed, so swap*/
                /* swap */
                tmp 			= heap[root].val;
                v   			= heap[root].comm_id;
								is_v_seed = heap[root].is_seed;
...
void aggc_maxheap_sift_down2(aggc_maxheap_t *maxheap, attr_id_t idx) {
    ...
    if (!maxheap->use2) {
        ...
        while (2*root+1 < n) {
            child = 2*root+1;
            if (child+1 < n) {
							/* if both the children are seed or neither them is a seed, take the
							   maximum of the two */
							if ((heap[child].is_seed == 1 && heap[child+1].is_seed == 1)
									|| (heap[child].is_seed == 0 && heap[child+1].is_seed == 0)){
                if (heap[child].val < heap[child+1].val)
                    child++;
							}
							/* if the second child is a seed, choose that as a potential candidate
								 for swap with the root */
							else if (heap[child].is_seed == 0 && heap[child+1].is_seed == 1)
										child ++;
						}
						/* if both root and child are seed vertices or if neither is a seed vertex
							 swap them based on value  */
						if (((heap[root].is_seed == 1 && heap[child].is_seed == 1)
								|| (heap[root].is_seed == 0 && heap[child].is_seed == 0)) &&
                ...
                root = child;
						...
						else if (heap[root].is_seed == 0 && heap[child].is_seed == 1) {
							/* since child is one of the seed vertices and the root is not, so swap */
                tmp 			= heap[root].val;
                v  				= heap[root].comm_id;
								is_v_seed = heap[root].is_seed;
                u  				= heap[child].comm_id;
...
void aggc_maxheap_remove2(aggc_maxheap_t *maxheap, attr_id_t comm_id) {
    ...
    if (!maxheap->use2) {
        ...
        heap[idx].val     = heap[n-1].val;
				heap[idx].is_seed = heap[n-1].is_seed;
        ...
        /*if (fpGT(heap[idx].val,old_val))*/
            aggc_maxheap_sift_up2(maxheap, idx);
        /*else*/
            aggc_maxheap_sift_down2(maxheap, idx);

#ifdef DEBUG_MAXHEAP
        fprintf(stderr,"DEBUG_MAXHEAP Checking heap: maxheap_remove end\n");
        aggc_maxheap_check2(maxheap);
#endif
...
attr_id_t aggc_adjcomm_pos2(aggc_adjcomm_t *adjcomm, attr_id_t val, attr_id_t degree) {
...
void aggc_adjcomm_resize2(aggc_comm_t *communities, attr_id_t comm_id, attr_id_t incr) {
...
void aggc_update_values2(aggc_comm_t *communities, attr_id_t comm1, attr_id_t idx,
...
void aggc_update_dq_p12(aggc_comm_t *communities, aggc_maxheap_t *maxheap,
    ...
    if (comm2 == comm1_max_key_adjcomm_id) {
...
#endif
        aggc_update_values2(communities, comm1, comm1_max_key_idx, comm2, L, maxheap->n - 1);
        ...
        if ( (fpGT(comm1_max_key,comm1_max_key_old)) 
            ...
            aggc_maxheap_sift_up2(maxheap, heap_pos);
        else
            aggc_maxheap_sift_down2(maxheap, heap_pos);
        ...
        pos = aggc_adjcomm_pos2(comm1_adjcomm, comm2, comm1_degree);
        ...
        fprintf(stderr,"DEBUG_MB bef UV %2d %2d p1-else pos: %2d\n",
                comm1, comm2, pos);
#endif
        aggc_update_values2(communities, comm1, pos, comm2, L, maxheap->n - 1);
...
void aggc_update_dq_p22(aggc_comm_t *communities, aggc_maxheap_t* maxheap,
...
#endif
        aggc_update_values2(communities, comm1, comm1_max_key_idx, new_comm_id, L, maxheap->n - 1);
...
#endif
        aggc_update_values2(communities, comm1, i, new_comm_id, L, maxheap->n - 1);
#endif
...
void aggc_remove_adjcomm2(aggc_comm_t *communities, aggc_maxheap_t *maxheap,
    ...
    if (comm2 == max_key_adjcomm_id) {
        ...
        pos = aggc_adjcomm_pos2(adjcomm, comm2, degree);
        if (max_key_idx > pos)
...
void aggc_compute_dqnorm_stats2(aggc_comm_t *communities, aggc_maxheap_t *maxheap, 
...
double aggc_merge_communities2(aggc_comm_t *communities, attr_id_t seed, 
    ...
    if (comm1 != communities[comm2].adjcomm[communities[comm2].max_key_idx].comm_id) {
        ...
        ins_pos = aggc_adjcomm_pos2(communities[comm2].adjcomm, comm1, communities[comm2].degree);
    ...
    aggc_maxkey_comm_check2(communities, to);
    ...
    while (i<to_deg && j<from_deg) {
        ...
        if (p1 < p2) {
            ...
            fprintf(stderr,"DEBUG_MB bef UV %2d %2d mc p1<p2 to: %2d i: %2d  p1: %2d\n",
                    to, p1, to, i, p1);
#endif
            aggc_update_values2(communities, to, i, p1, *L, maxheap->n - 1);
            ...
            aggc_update_dq_p12(communities, maxheap, p1, to, *L, to_adj[i].value[KEY_CNM], updated_weight);
            i++;
            curr_i_idx++;
            continue;
        }
        ...
        if (p1 > p2) {
            ...
            if (communities[to].degree == communities[to].max_size) {
                aggc_adjcomm_resize2(communities, to, from_deg);
                to_adj = communities[to].adjcomm;
            } 
            ...
            fprintf(stderr,"DEBUG_MB bef UV %2d %2d mc p1>p2 to: %2d ins_pos: %2d  p2: %2d\n",
                    to, p2, to, ins_pos, p2);
#endif
            aggc_update_values2(communities, to, ins_pos, p2, *L, maxheap->n - 1);
            ...
            aggc_update_dq_p22(communities, maxheap, p2, from, to, *L, to_adj[ins_pos].value[KEY_CNM], updated_weight);


#ifdef DEBUG_MAXKEY
            /* Make sure we haven't violated p2's maxkey! */
            fprintf(stderr,"DEBUG_MAXKEY Maxkey check for p2.\n");
            aggc_maxkey_comm_check2(communities, p2);
#endif
    ...
    qsort(communities[to].adjcomm, communities[to].degree, sizeof(aggc_adjcomm_t), comp_comm2);
    ...
    aggc_maxheap_remove2(maxheap, from);
...
attr_id_t aggc_compute_membership2(aggc_comm_t *communities, attr_id_t *membership, attr_id_t n) {
...
void seed_set_community_detection(graph_t *g, char *alg_type,
		attr_id_t* seeds, attr_id_t n_seeds, int uniform, 
	...
	fprintf (stderr, "*** Seed set community detection ***\n");
	assert (seeds != NULL && n_seeds > 0);
#if 0
	fprintf (stderr, "Input seeds	:");
	for (i=0; i <n_seeds; i++)
		fprintf (stderr, "%d ", seeds[i]);
	fprintf (stderr, "\n");
#endif
		qsort (seeds, n_seeds, sizeof(int), comp_int2);
#if 0
	fprintf (stderr, "Sorted seeds	:");
	for (i=0; i <n_seeds; i++)
		fprintf (stderr, "%d ", seeds[i]);
	fprintf (stderr, "\n");
#endif
    ...
    for (i=0; i<n; i++) {
        ...
        qsort(comm_memchunk+start_iter, v_degree_i, sizeof(aggc_adjcomm_t), comp_comm2);
        uniq_adj_count = 0;
        if (v_degree_i > 0) {
...
#ifdef DEBUG
    aggc_print_communities2(communities, n, L);
#endif
    ...
    for (i=0; i<n; i++) {
        ...
        heap[maxheap->n].comm_id = i;
				...
				/* if community 'i' is a seed vertex set heap[maxheap->n].is_seed=1 */
				if (j < n_seeds && seeds[j] == i) {
						heap[maxheap->n].is_seed = 1;
						j++;
				}
				else
						heap[maxheap->n].is_seed = 0;
...
#endif
        aggc_maxheap_sift_up2(maxheap, maxheap->n);
        maxheap->n++;
    }
...
#ifdef DEBUG_MAXHEAP
    for (i=0; i<maxheap->n; i++) {
        fprintf(stderr, "DEBUG_MAXHEAP maxheap->heap[%4d].is_seed (%d), val (%20f) "
#ifdef DO_HEAP2
                "val2 (%20f) "
#endif
                "comm_id (%4d)\n", 
                i, maxheap->heap[i].is_seed, maxheap->heap[i].val,
#ifdef DO_HEAP2
                maxheap->heap[i].val2,
#endif
    ...
    for (i=0; i<maxheap->n; i++) {
        fprintf(stderr, "maxheap->heap[index[%4d] (%4d)].comm_id: %4d .is_seed: %d\n", i, index[i], maxheap->heap[index[i]].comm_id, maxheap->heap[index[i]].is_seed);
    }
#endif
...
#if 1
		fprintf(stderr, "Total joins: %d\n", total_joins);
#endif

	attr_id_t next_seed_idx = -1;
    while (no_of_joins < total_joins) {
		/*
		 * Cycle around the seeds so that there is a
		 * uniform growth of each seed community before
		 * the merge
		 */
#if 0
	for (i=0; i <maxheap->n; i++)
	...
	fprintf(stderr, "\n");
	for (i=0; i <maxheap->n; i++) {
		fprintf (stderr, "%3d ", maxheap->index[i]);
	}
	fprintf(stderr, "\n");
#endif
		...
		if (uniform == 1) {
			do {
				next_seed_idx = (next_seed_idx + 1) % n_seeds;
				seed = seeds[next_seed_idx];
			}while (maxheap->index[seed] == -1);
#if 0
	fprintf (stderr, "Next seed chosen (%d): %d\n", next_seed_idx, seed);
#endif
		}
				
				if (maxheap->n >= 3) {
					if (heap[0].is_seed == 1 && heap[1].is_seed == 0 && heap[2].is_seed == 0) {
						fprintf (stderr, "Terminating while loop: All the seeds have come together in a single community.\n");
						break;
					}
				} else if (maxheap->n == 2) {
					if (heap[0].is_seed == 1 && heap[1].is_seed == 0) {
						fprintf (stderr, "Terminating while loop: All the seeds have come together in a single community.\n");
						break;
					}
				} else if (maxheap->n == 1) {
					fprintf (stderr, "Terminating while loop: Left with only a single vertex.\n");
					break;
				}
...
#ifdef DEBUG_MAXKEY
        aggc_maxkey_check2(communities,maxheap);
#endif

        new_max_key = aggc_merge_communities2(communities, seed, maxheap, adj_buffer, n - no_of_joins, &L);

#if 0
        final_num_communities = aggc_compute_membership2(communities, membership, n);
        for (i=0 ; i<n ; i++) 
            if (g->zero_indexed)
    ...
    fprintf(stderr, "\n");
    /*  aggc_print_communities2(communities, n, L); */
    aggc_print_final_communities2(communities, maxheap, L);
#endif
...
/**
 * Identifies the community around a single seed. It invokes the
 * _modularity_greedy_agglomerative function in modularity_greedy_agglomerative.c
 * with a single seed. Adjacent communities are merged to this seed one at a time
 * to identify the community around this seed.
 */
void single_seed_community_detection (graph_t *g, char *alg_type, attr_id_t seed, 
        attr_id_t *membership, 
        attr_id_t *num_communities, 
        double *modularity) {
	fprintf (stderr, "*** Single seed community detection ***\n");
