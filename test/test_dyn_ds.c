...
#pragma omp parallel for schedule(static) num_threads(24)
    ...
    for (ti=0; ti<nexp; ti++) {
#pragma omp parallel num_threads(nt[ti])
#endif
{
    attr_id_t i, n, m;
    attr_id_t u, v, eid;
...
#endif
#pragma omp parallel num_threads(nt[ti]) //reduction(+:num_mallocs) reduction(+:num_init_mallocs)
#endif
{
    attr_id_t i, n, m;
    attr_id_t u, v, eid;
...
#ifdef _OPENMP
#pragma omp for schedule(dynamic, chnk_sz)
#endif
    for (i = 0; i < m; i++) {
        u = src[i];
        v = dest[i];
